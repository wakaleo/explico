...
class CoverageTest {
    ...
    fun ofRuleGroupSumsAcrossAllItsBodies() {
        val rule = RuleGroup(
            ...
            bodies = listOf(
                RuleBody(listOf(Condition.Comparison(path, Operator.EQ, literal)), null, SourceRef("f.rego", 1)),
                RuleBody(listOf(Condition.Unrendered("x", "unclassified")), null, SourceRef("f.rego", 5)),
            ),
        )
    ...
    fun ofPolicyPackageSumsAcrossAllItsRules() {
        val ruleA = RuleGroup("a", null, null, listOf(RuleBody(listOf(Condition.Comparison(path, Operator.EQ, literal)), null, SourceRef("f.rego", 1))))
        val ruleB = RuleGroup("b", null, null, listOf(RuleBody(listOf(Condition.Unrendered("x", "unclassified")), null, SourceRef("f.rego", 5))))
